...
import requests #tuodaan requests-moduuli ohjelmaan
from bs4 import BeautifulSoup #tuodaan beautifulsoup moduuli
from tkinter.font import Font #tuodaan fonttikirasto.
...
def getIndex():
    ...
    page = requests.get('https://www.arvopaperi.fi/')
    soup = BeautifulSoup(page.text, 'html.parser')
    #etsitään parametrina oleva div-luokka nettisivulta
    result = soup.find(class_='vnruoc-0 fYBDWm')
    #haetaan kaikki divin sisältä tieto
    result2 = result.find_all('div')
...
indexbtn = Button(frame5, text = 'Check indexes & currency rates', command = getIndex)
